...
<x-common.page-breadcrumb pageTitle="Tambah Data Table C" />
...
<x-common.component-card title="Form Input Manual">
    <form method="POST" action="{{ route('table-c.store') }}" class="space-y-5">
        @csrf
        ...
        <div>
            <label class="text-sm font-medium">Area Sales</label>
            <input
                type="text"
                name="area_sales"
...
<div id="importCsv" class="{{ session('previewData') || session('error') ? '' : 'hidden' }} mt-6">
<x-common.component-card title="Import CSV Table C">

    <form
        method="POST"
        action="{{ route('table-c.import.preview') }}"
        ...
        <p class="text-sm text-gray-500">
            Header wajib: <b>kode_toko,area_sales</b>
        </p>
    ...
    @if (session('previewData'))
        ...
        <form method="POST" action="{{ route('table-c.import.confirm') }}">
            @csrf
                ...
                <table class="min-w-full border rounded-lg">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-2 text-center text-sm">Area Sales</th>
